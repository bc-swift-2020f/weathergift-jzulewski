...
class PageViewControlller: UIPageViewController {
    ...
    func loadLocations() {
        guard let locationsEncoded = UserDefaults.standard.value(forKey: "weatherLocations")
                as? Data else {
                print("Warning: Could not load weatherLocations data from UserDefaults. This would happen the first time the app is installed. If that's the case ignore this warning")
    ...
    {
        let detailViewController = storyboard!.instantiateViewController(withIdentifier: "LocationDetailViewController") as!
...
extension PageViewControlller: UIPageViewControllerDelegate, UIPageViewControllerDataSource {
